  ...
  export default {
    ...
    created(){
    	...
    	this.$get('student_papers/'+this.paperId,{"studentId":this.studentId}).then((res) => {
	      	if(res.code === 0){
	      		// console.log(res);
	      		this.readingWriting = res.data.readingWriting[9];
	      		this.writeTestName = res.data.readingWriting[9].testName 
	      		this.writelist = JSON.parse(JSON.stringify(res.data.readingWriting[9].itemList))
	      		// var readingWriting = res.data.readingWriting[9]; 
	      		// console.log(readingWriting);
	      		// if(readingWriting){
		      	// 	for(var i=0,len = res.data.writing.testList.length ; i< len;i++){
		      	// 		res.data.writing.testList[i].exam = ""
		      	// 	}
		      		// this.writeTestName = res.data.readingWriting.testName
        ...
        var	item,count = 0;
      	for(var i=0; i <this.writelist.length; i++ ){
      		if(this.writelist[i].exam === ""){
    			...
    			item = {"scoreId":this.writelist[i].scoreId,"score":parseInt(this.writelist[i].exam)? this.writelist[i].exam:''}
    			this.exam.push(item)
      	}
      	if(count === 1){
